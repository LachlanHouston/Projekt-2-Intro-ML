...
for train_partition, test_partition in outer_Kfold.split(X,y):
    ...
    for n_idx, n in enumerate(hidden_units):
        ...
        innernet, final_loss, learning_curve = train_neural_net(inner_model,
                                                    loss_fn,
                                                    X=X_train_tensor,
                                                    y=y_train_tensor,
                                                    n_replicates=1,)
        ...
        y_test_est_inner = innernet(X_test_tensor)
        se = (y_test_est_inner.float()-y_test_tensor.float())**2 # squared error
        mse = (sum(se).type(torch.float)/len(y_test_est_inner)).data.numpy() #mean
        ANN_n_inner_error[inner_fold_counter,n_idx] = mse

    # ANN Outer
    mean_error_of_inner_h = np.mean(ANN_n_inner_error,axis = 0)
    best_idx = np.argmin(mean_error_of_inner_h)
    best_h = hidden_units[best_idx]
    ...
    outernet, final_loss, learning_curve = train_neural_net(outer_model,
                                                loss_fn,
                                                X=X_train_tensor,
                                                y=y_train_tensor,
                                                n_replicates=1,)
        
    y_test_est_outer = outernet(X_test_tensor)
...
#%%
modError_All = np.array([error_test_baseline, error_test_linreg, ANN_outer_MSE]).squeeze()
np.savetxt("errors.csv", modError_All, delimiter=",")

genError_All = np.array([baseline_outer_err, linreg_outer_err, ANN_err])
np.savetxt("allerrors.csv", genError_All, delimiter=",")
